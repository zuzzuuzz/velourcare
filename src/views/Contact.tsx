...
function ContactContent() {
  ...
  const enquiryType: "trial" | "meeting" | "general" =
    rawType === "meeting" ? "meeting" : rawType === "general" ? "general" : "trial";

  return (
    <div id="contact-page" className="pb-20 bg-white space-y-16">
      {/* Page Header */}
      <section className="bg-slate-900 text-white py-20 sm:py-24 relative overflow-hidden">
        ...
        <Reveal className="layout-container relative z-10 text-center space-y-4">
          ...
          <h1 className="font-display font-black text-4xl sm:text-5xl lg:text-6xl text-white tracking-tight leading-none">
            Clinical Lab Portal & Inquiry Form
          </h1>
          <p className="text-slate-300 text-sm sm:text-base max-w-2xl mx-auto leading-relaxed">
            Submit a trial case, schedule a clinical consultation, or reach our orthodontic planning team in Lahore.
          </p>
        </Reveal>
      </section>

      {/* Interactive Inquiry & Case Submission Form */}
      <section id="form" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal variant="scale" className="w-full">
          <InteractiveForm key={enquiryType} defaultEnquiryType={enquiryType} />
        </Reveal>
      </section>

      {/* Direct Information List & Office Location */}
...
export default function Contact() {
  return (
    <Suspense fallback={<div className="py-24 text-center text-slate-500 font-semibold">Loading portal...</div>}>
      <ContactContent />
    </Suspense>
  );
}
